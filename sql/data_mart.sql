...
-- 2.1.Tạo các bảng cần thiết để insert dữ liệu nếu chúng chưa tồn tại
-- Bảng `dim_province`
...
-- Bảng `staging_lottery` (Staging area) để chứa dữ liệu từ nguồn `lottery_db` (ETL)
...
-- Bảng `lottery_prize_values` lưu trữ thông tin về các giá trị của giải thưởng
...
-- 2.2.Insert dữ liệu vào các bảng
-- 2.2.1    .Nhập dữ liệu trực tiếp, insert giải thưởng vào bảng `lottery_prize_values`,
...
-- 2.2.2.Insert từ nguồn
-- Insert dữ liệu từ nguồn `lottery_db` vào bảng `staging_lottery`
...
SELECT p.`name`                                                                AS `province`, -- Cột `name` trong bảng `province` thay cho `province_name`
       STR_TO_DATE(CONCAT(d.`year`, '-', d.`month`, '-', d.`day`),
                   '%Y-%m-%d')                                                 AS `date`,     -- Kết hợp year, month, day thành một ngày hợp lệ
...
-- Insert dữ liệu từ `lottery_db.province` vào `dim_province`
INSERT INTO `dim_province` (`province_name`)
SELECT DISTINCT `name`
FROM `lottery_db`.`province`;

-- Insert dữ liệu từ `lottery_db.date` vào `dim_date`
INSERT INTO `dim_date` (`day`, `month`, `year`)
SELECT `day`, `month`, `year`
FROM `lottery_db`.`date`;

-- Insert dữ liệu vào bảng `dim_lottery_prize`
...
-- 2.2.2.3 Insert dữ liệu từ bảng `lottery_db.lottery` và các bảng dimension vào `fact_lottery`
...
-- 2.2.2.4.Tạo bảng `summary_lottery` để tóm tắt dữ liệu
...
-- Update bảng `summary_lottery` từ bảng `fact_lottery`
...
-- Insert dữ liệu mới vào `summary_lottery` nếu chưa tồn tại
